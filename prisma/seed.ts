...
async function seedBookItems() {
  ...
  // Sinh ngẫu nhiên từ 1 đến 100 cuốn vật lý (BookItem) cho mỗi đầu sách
  for (let bookId = 1; bookId <= bookCount; bookId++) {
    const bookItems = [];

    // Đẻ ra 1 số ngẫu nhiên từ 1 đến 100
    const randomQuantity = Math.floor(Math.random() * 100) + 1;

    for (let itemNumber = 1; itemNumber <= randomQuantity; itemNumber++) {
      // Chỉnh padStart thành 3 số cho đẹp vì số lượng lên tới 100 (VD: BK001-099)
      const code = `BK${bookId.toString().padStart(3, '0')}-${itemNumber.toString().padStart(3, '0')}`;
    ...
    // Đẩy dữ liệu vào DB theo TỪNG CUỐN SÁCH để tránh làm sập Database
