...
export const POST = requireLibrarian(async request => {
  try {
    ...
    // Validate Publish Year (cannot be in the future)
    if (publishYear) {
      const year = Number(publishYear);
      const currentYear = new Date().getFullYear();
      if (year > currentYear) {
        throw new ValidationError(`Publish year cannot be in the future (max: ${currentYear})`);
      }
    }
    ...
    const existingBook = await prisma.book.findFirst({
      where: { title: sanitizedTitle, isDeleted: false }
    });
