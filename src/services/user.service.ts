...
import { EmailUtils, PasswordUtils } from '@/lib/utils';
...
export class UserService {
  ...
  static async updateUser(id: number, userData: Prisma.UserUpdateInput): Promise<PublicUser> {
    ...
    if (newEmail && !EmailUtils.isValid(newEmail)) {
    ...
    if (userData.phoneNumber) {
      ...
      } else if (userData.phoneNumber.set) {
        newPhoneNumber = userData.phoneNumber.set;
      }
    }

    // Simple validation for phone number (only digits, 10-15 length)
    if (newPhoneNumber) {
